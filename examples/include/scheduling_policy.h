...
namespace jampi {
    ...
    template<class Algorithm, class THREAD = void, class AssertCompatible = typename std::enable_if<std::is_base_of<typename jampi::SchedAlgorithm, Algorithm>::value>::type>
    class SchedulingPolicy {
    private:
        Algorithm scheme;
        std::mutex thread_table_mutex;
        std::atomic<int> context_allocate;
        std::unordered_multimap<int, jampi::ThreadInterface *> thread_table;
    public:
