...
#include "member_callable_impl.h"
#include "task_interface.h"
#include "overload_tag.h"
...
// porem nao serve em C++11, foco do trabalho).
    ...
    class Task : public virtual jampi::TaskInterface {
        ...
        void apply(seq<index...>, jampi::OverloadTag<T>) {
            if (function) return_channel.set_value((*function)(std::get<index>(arguments)...));
        }

        template<int ...index>
        void apply(seq<index...>, jampi::OverloadTag<void>) {
            if (function) {
    ...
    public:
        ...
        void operator()() {
            apply(typename gens<sizeof...(Args)>::type(), jampi::OverloadTag<ReturnType>{});
        }
